						...
						.map((previousAppointment) => (
							<div key={previousAppointment._id} className="singled-doctor">
								...
								{previousAppointment.source === "Completed" && (
									<>
										{previousAppointment.rating != null ? (
											<ul>
												<li>
													<strong>Your Review: </strong> {previousAppointment.review}
												</li>
												<li>
													...
													<span style={{ display: "inline-flex", gap: "2px" }}>
														{[...Array(5)].map((_, i) => (
															<Star
																key={i}
																size={18}
																color={i < previousAppointment.rating ? "#ffc107" : "#e4e5e9"}
															/>
														))}
													</span>
												</li>
											...
											<button
												className="action-button"
												onClick={() => navigate(`/PatientFeedback/${previousAppointment._id}`)}
											>
												Give your Rating
											</button>
										)}
